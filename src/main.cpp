    ...
    while (1) 
    {
        ...
        while (size_t bytes_read = read(client_fd, req_buf, 1024))
        {
            ...
            if (request_api_key == 0x004b) // DescribeTopicPartitions
            {
                ...
                *ptr++ = topic_length;
                // std::cout << "topic_length: " << topic_length << '\n';
            ...
            if (request_api_key == 0x0012) // API Versions
            {
                ...
                write_int16_be(&ptr, error_code);
                int8_t num_api_keys = 1 + 4; // 1 + # of elements because 0 is null array and 1 is empty array
                *ptr++ = num_api_keys;
                ...
                *ptr++ = TAG_BUFFER;
                ...
                write_int16_be(&ptr, 0); // api_key ( PRODUCE )
                write_int16_be(&ptr, 0);  // min_ver
                write_int16_be(&ptr, 11);  // max_ver
                *ptr++ = TAG_BUFFER;
